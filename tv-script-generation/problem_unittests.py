...
def test_build_nn(build_nn):
    with tf.Graph().as_default():
        ...
        final_state_expected_shape = [test_rnn_layer_size, 2, test_input_data_shape[0], test_rnn_size]
        #print('final_state_expected_shape: {}'.format(final_state_expected_shape))
        ...
        assert final_state.get_shape().as_list() == final_state_expected_shape, \
            'Final state wrong shape.  Found shape {}'.format(final_state.get_shape())
